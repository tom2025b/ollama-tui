...
fn explain_with_code_block_sends_prompt_to_model() {
    ...
    let request = app.submit_prompt().expect("explain should produce a model request");

    assert!(request.prompt.contains("Explain the following code"));
...
fn review_with_code_block_sends_prompt_to_model() {
    ...
    let request = app.submit_prompt().expect("review should produce a model request");
...
fn fix_prefers_code_block_over_assistant_message() {
    ...
    let request = app.submit_prompt().expect("fix should produce a model request");

    assert!(request.prompt.contains("Find and fix all bugs"));
...
fn fix_falls_back_to_last_assistant_message_when_no_code_block() {
    ...
    assert!(request.prompt.contains("Find and fix all factual or logical mistakes"));
