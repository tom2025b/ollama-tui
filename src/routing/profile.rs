...
use keywords::{
    COMPLEX_WORK_KEYWORDS, CREATIVE_OR_GENERAL_TERMINAL_KEYWORDS, CURRENT_CONTEXT_KEYWORDS,
    DEEP_REASONING_OR_CODE_KEYWORDS, LOCAL_ONLY_KEYWORDS, SENSITIVE_PHRASES, SENSITIVE_WORDS,
    SIMPLE_KEYWORDS,
};
...
pub(super) struct PromptProfile {
    ...
    pub(super) is_creative_or_general_terminal: bool,
}
...
impl PromptProfile {
    ...
    pub(super) fn from_prompt(prompt: &str) -> Self {
        ...
        let needs_current_context = contains_any(&prompt_lowercase, CURRENT_CONTEXT_KEYWORDS);
        let needs_deep_reasoning_or_code =
            contains_any(&prompt_lowercase, DEEP_REASONING_OR_CODE_KEYWORDS)
                || contains_any(&prompt_lowercase, COMPLEX_WORK_KEYWORDS)
                || word_count >= 40;
        let is_creative_or_general_terminal =
            contains_any(&prompt_lowercase, CREATIVE_OR_GENERAL_TERMINAL_KEYWORDS);
        let is_simple = (word_count <= 20 || contains_any(&prompt_lowercase, SIMPLE_KEYWORDS))
            && !needs_current_context
            && !needs_deep_reasoning_or_code
            && !is_creative_or_general_terminal;
